...
import { addRepairUpdate, createEstimate, sendStaffMessage } from "@/app/actions/repair-details";
import { regenerateClientAccess, revokeClientAccess } from "@/app/actions/repairs";
...
export default async function RepairDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await db.repairOrder.findUnique({ where: { id }, include: { customer: true, device: true, branch: true, receivedBy: true, technician: true, updates: { orderBy: { sequence: "desc" }, include: { user: true } }, messages: { orderBy: { createdAt: "asc" }, include: { senderUser: true } }, estimates: { orderBy: { version: "desc" } }, photos: true } });
  if (!order) notFound();
  ...
  return <>
    ...
    <div className="grid-two" style={{marginTop:20}}><section className="card"><h3>Registrar avance</h3><form action={addRepairUpdate.bind(null,order.id)} className="form-grid one"><div className="field"><label>Nuevo estado</label><select name="status" defaultValue={order.status}><option value={order.status}>{repairStatusLabels[order.status]} (sin cambio)</option>{transitions.map((s)=><option value={s} key={s}>{repairStatusLabels[s]}</option>)}</select></div><div className="field"><label>Comentario visible para cliente</label><textarea name="comment" required/></div><div className="field"><label>Diagnóstico actualizado</label><textarea name="diagnosis" defaultValue={order.diagnosis || ""}/></div><button className="btn btn-primary">Guardar avance</button></form></section><section className="card"><h3>Línea de tiempo</h3><div className="timeline">{order.updates.map((u)=><div className="timeline-item" key={u.id}><strong>{repairStatusLabels[u.newStatus]}</strong><span>{u.comment}</span><small>{u.user.name} · {u.createdAt.toLocaleString("es-MX")}</small></div>)}</div></section></div>
