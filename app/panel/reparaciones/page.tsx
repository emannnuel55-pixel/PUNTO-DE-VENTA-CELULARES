...
export default async function RepairsPage() {
  ...
  return <>
    ...
    <section className="card"><div className="table-wrap"><table><thead><tr><th>Folio</th><th>Cliente</th><th>Equipo</th><th>Problema</th><th>Estado</th><th>Total</th><th>Técnico</th><th></th></tr></thead><tbody>{repairs.map((r) => <tr key={r.id}><td><strong>{r.publicFolio}</strong><br/><small>{r.createdAt.toLocaleString("es-MX")}</small></td><td>{r.customer.firstName} {r.customer.lastName}<br/><small>{r.customer.phone}</small><br/><small style={{color:"#3b82f6", fontWeight:"bold"}}>Código: {r.accessCodePlain || `••••-${r.accessCodeLast4}`}</small></td><td>{r.device.brand} {r.device.model}<br/><small>{r.device.serialNumber || r.device.imei || "Sin serie"}</small></td><td style={{maxWidth:250}}>{r.issue}</td><td><span className="badge">{repairStatusLabels[r.status]}</span></td><td>{formatMoney(r.total.toString())}</td><td>{r.technician?.name || "Sin asignar"}</td><td><Link className="btn btn-small btn-secondary" href={`/panel/reparaciones/${r.id}`}>Abrir</Link></td></tr>)}</tbody></table></div></section>
  </>;
}
